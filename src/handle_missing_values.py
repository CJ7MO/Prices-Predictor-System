...
class DropMissingValuesStrategy(MissingValueHandlingStrategy):
    def __init__(self, axis=0, thresh=None):
        ...
        Parameters:
        axis(int): 0 to drop rows with missing values, 1 to dop columns with missing values.
        thresh(int): The threshold for non-NA values. Rows/Columns with less thresh non-NA values are dropped.
        """
        self.axis = axis
        self.thresh = thresh
    ...
    def handle(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Drops rows or column with missing values based on the axis and threshold.
...
class FillMissingValuesStrategy(MissingValueHandlingStrategy):
    def __init__(self, method="mean", fill_value=None):
        """
        Initializes the FillingMissingValuesStrategy with a specific method or fill value.
...
class MissingValueHandler:
    ...
    def handle_missing_values(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Executes the missing values handling using the current strategy.
